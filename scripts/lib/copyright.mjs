 ...
 * test/copyright.test.mjs import and exercise these functions without the CLI's
 * file walk rewriting the working tree on import.
 *
 ...
 * block comment early — the same class of delimiter confusion the CSS fix below
 * exists to prevent.)
 *
...
// (postcss, and therefore stylelint, included) when it is the very first three
// bytes of the file on disk — which is what prependCssComment() below relies on
// to keep it there. Harmless ECMAScript WhiteSpace to node wherever it lands
// instead, so prependJsDocblock() treating it the same way is for consistency
// across the function family, not out of necessity. Written as an escape rather
// than a literal U+FEFF so the source carries no invisible characters.
const BOM = '\uFEFF';
...
// Return the carriage return that has to be appended to any line WE inject, so
// an injected line does not end up LF-terminated inside an otherwise CRLF file.
// `content.split('\n')` leaves the '\r' at the end of each original line, so
// re-joining with '\n' reproduces CRLF for untouched lines — only injected lines
// need this. Matters here because .prettierrc.json pins `endOfLine: "lf"` and
// prettier --check covers sites/**/*.css, so a stray CR would be a lint failure
...
// in an otherwise all-LF file is enough to make the injected line CRLF too,
// regardless of what its immediate neighbour lines use. On a genuinely mixed-EOL
// input that can leave the injected line's terminator matching neither the line
// before nor the line after it. That is intentional-by-omission (simplicity over
// a rare edge case), not a bug to silently "improve" here.
...
//
// If `content` starts with a BOM the docblock must still land AFTER it, not
// before. U+FEFF is ECMAScript WhiteSpace, so a relocated BOM here is benign to
// node (unlike the CSS case, where it joins the following selector and silently
// drops the rule) — but leaving one path fixed and the other not is an
// inconsistency in the same function family, so it is handled here too.
...
// injectJsDocblock() above does NOT have this defect: it only splices a new line
// into the array and never touches line 0, so a leading BOM already stays
...
function prependJsDocblock(content) {
  ...
  const docblock = ['/**', ' * Phlix brand kit configuration.', ' *', COPYRIGHT, ' */', ''];
...
//
// Finds the terminator of the OPENING block comment — the FIRST `*/` at or after
// the opening `/*` (scanning from the top of the file, since the opening comment
// always starts on line 0) — and inserts the copyright line just before it, so
// it stays inside that block.
//
// Deliberately NOT the last `*/` in the whole file. That is the defect this
// function was ported to fix: the previous implementation looped over every line
// WITHOUT breaking, keeping the LAST index that contained `*/`. Every site
// stylesheet in this repo is full of later `/* ... */` comments — section
// banners, `/* stylelint-disable ... */` pragmas, and machine markers such as
...
//      prependCssComment().
//
//   2. ALREADY HAS THE HEADER: the content carries the marker ANYWHERE (not just
//      inside the opening comment), checked before any comment-shape reasoning
//      even runs. The caller MUST do NOTHING. Calling prependCssComment() here
//      produces a SECOND header.
//
...
function injectCssComment(content) {
  ...
  // but the function is exported, so its own guard must not be narrower than the
  // file it is handed).
  ...
  // Find the terminator of the opening block comment: the first `*/` at or after
  // `openAt + 2`. Starting the scan past the opening delimiter matters for a
  // degenerate first line like `/*/`, where the `*` is SHARED between the opener
  // and the apparent closer — `'/*/'.indexOf('*/') === 1`, so a naive scan would
  // "close" the comment on top of its own opener and split the line into a bare
  // `/`, destroying a source character. There is no terminator there, so we keep
  // scanning (and, absent one, return null and let the caller prepend a valid
  // header, leaving the input intact).
  let closeIdx = -1;
  let closeAt = -1;
  ...
  if (closeIdx === 0) {
    ...
    // Multi-line opening comment — insert copyright just before the closing line
    // so it stays inside the block.
...
//
// If `content` starts with a BOM the header must still land AFTER it, not before
// — prepending in front of the BOM leaves it sitting mid-file, in front of the
// first selector instead of in front of the file, and postcss then parses that
...
export { injectCssComment, prependCssComment, injectJsDocblock, prependJsDocblock, COPYRIGHT, MARKER };
