...
_experience_ fields — `site_architecture`, `homepage_narrative`, `page_blueprints`,
...
whole purpose is to make each kit a genuinely different _product experience_, not
a palette swap.
...
Each kit's experience fields change _structure_: nav, section order, page
...
where the line landed _inside_ an existing banner comment — valid CSS, untidy —
and the fact that headers now sit mid-file in 146 CSS files and at line 1 in only 4.
...
| `detain/phlix-server/blob/master/LICENSE`                   | 33    | ~~That repo has no LICENSE file~~ — **RESOLVED 2026-07-24**: phlix-server now ships an MPL-2.0 `LICENSE`; the URL returns 200. See §0.9.                 |
...
### 0.9 The published license claim contradicted the code — RESOLVED 2026-07-24

**Resolved. No longer blocking.** The owner settled the licensing on 2026-07-24 and
the tiering below is now real in every repo:
...
required — no OSI licence can compel a visible credit). `shared/content.json` was
updated to match: the FAQ answer and the footer label now state MPL-2.0/MIT.

**What regen authors must do:** take the licence wording from `content.json`, never
from memory, and never state one licence "across the board". `new_site.md` §16 and
§19.7 now say so explicitly.

<details><summary>The original defect, for the record</summary>

All 50 sites stated BSD-3-Clause as **fact**, including a FAQ answer sourced from
`content.json`: _"What's the license?" → "BSD-3-Clause across the board."_ The
repos said otherwise:

| Repo             | Declares                                                                                                                          |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **phlix-server** | **`"license": "proprietary"`** in `composer.json`, and README: _"Proprietary - All rights reserved."_ **No LICENSE file exists.** |
| phlix-hub        | MIT                                                                                                                               |
| phlix-shared     | MIT                                                                                                                               |
...
1. Does a regen pass driven by `regen_site_prompt.md` produce a site that is
   _visibly_ a different experience from its July-4 predecessor — different nav,
...
1. **Experience fidelity.** Every experience field the kit declares is _observably_
