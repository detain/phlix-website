...
(function () {
  ...
  class CapturedManager {
    ...
    createCapturedNotification(itemData) {
      ...
      text.textContent = itemData.title
        ? `${itemData.title.toUpperCase()} CAPTURED`
        : 'PREY CAPTURED';
    ...
    triggerPounce() {
      ...
      requestAnimationFrame(() => {
        this.element.style.animation =
          'capturedSnap 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards';
        setTimeout(() => {
          this.element.style.animation = '';
        }, 500);
      });
  ...
  function init() {
    ...
    document.querySelectorAll('.prey-card').forEach((card) => {
      const itemId =
        card.dataset.id || card.querySelector('.prey-title')?.textContent || `item-${Date.now()}`;
