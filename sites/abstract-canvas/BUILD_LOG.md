...
| ~~Meta `description` is `content.json`'s on all 9 pages~~ — **RESOLVED**      | ROUND-1 #18. Eight pages now carry authored per-page descriptions; `index.html` keeps the canonical one. `check-meta` still passes on all 407 files.                                                                                                                                                                                                                                                                                                                                                                                                            |
| ~~Palette overlaps a corner of content at 320px~~ — **RESOLVED (twice)**      | ROUND-1 #3 measured what this row waved away: the companion _did_ cover "Set Up Your Studio" at five phone sizes. ROUND-1's answer was `display: none`, which ROUND-2 ❌1/❌2 correctly read as deleting the mascot rather than moving it. `js/main.js` `guardPalette` now walks the companion up its own edge to the nearest band no control occupies and never hides it: 0 absent and 0 overlaps across 931 scroll stops × 8 viewports × 9 pages. See `REGEN_PLAN.md` §8.                                                                                     |
...
| ~~Composer bootstrap in the install snippet~~ — **RESOLVED, and I was wrong** | This row argued that `git clone` + `composer install` was "the standard PHP bootstrap for this repo" and that a different command "belongs in `content.json`". The second half was right and the first half was not: a checkout creates no database, no service and no migrations, so shipping it under an "install" heading was a §16 honesty failure rather than a formatting choice. `shared/content.json` now carries an orchestrator-owned `install` block, and `download.html` uses `install.primary.command` verbatim — `curl -fsSL …/scripts/install.sh | sudo bash`. The checkout survives one disclosure below it, labelled as a development checkout. See `new_site.md`§19.22 and`REGEN_PLAN.md` §8.2. |

## 10. ROUND-1 fix pass — 2026-07-25

`reviews/abstract-canvas/ROUND-1.md` filed 8 ❌ and 18 ⚠️ against this build. All 8 and 16 of the 18
are fixed; the two that are not are shared-file escalations (no italic face in the font pool, no
favicon/PWA set) and are recorded in `REGEN_PLAN.md` §6.6 and §6.7. The change-by-change delta,
including the one budget I traded (`js/main.js` 13.9 → 16.3 KB against a 48 KB font saving), is
`REGEN_PLAN.md` §7. **The ROUND-2 pass (2 ❌ / 17 ⚠️) is `REGEN_PLAN.md` §8**, which also carries the
re-measured JS figure (27.2 KB) and the two findings declined with a reason. Verification: `selfcheck` PASS, `render-check` PASS (9 pages × 4 viewports +
a 200%-text-zoom pass over every page), `prettier`/`stylelint`/`eslint`/`htmlhint`/`check-meta` clean,
plus a purpose-built probe covering target sizes, logo-link activation by pointer and by `Enter`,
palette-vs-CTA geometry over 10 viewports, the three nav emphasis rungs, the seasonal motif under
`?season=`, the JS-disabled phone nav, and print output.
